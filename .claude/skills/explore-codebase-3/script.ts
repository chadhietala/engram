#!/usr/bin/env bun
/**
 * explore-codebase-3 - Auto-generated skill script
 * Generated by Engram from pattern: 477c9c94-e6e4-4a67-9316-92ec0b157bde
 *
 * This skill applies a two-phase exploration strategy to efficiently understand codebases by first mapping architecture breadth-first, then investigating key areas depth-first.
 */
...
async function step32() {
  const filePath = `${targetDir}/generate-skill.ts`;
  const content = await Bun.file(filePath).text();
  console.log(`Read ${content.length} bytes from ${filePath}`);
  return content;
}
...
async function main() {
  try {
    ...
    await step31();
    console.log("Step 32: Read file /Users/chietala/Code/engram/scripts/generate-skill.ts");
    await step32();
