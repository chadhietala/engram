#!/usr/bin/env bun
/**
 * build-comprehensive - Auto-generated skill script
 * Generated by Engram from pattern: 477c9c94-e6e4-4a67-9316-92ec0b157bde
 *
 * This skill performs deep codebase exploration through systematic reading and pattern-based file discovery to build comprehensive understanding before implementation.
 */
...
async function step21() {
  const filePath = `${targetDir}/script.ts`;
  const content = await Bun.file(filePath).text();
  console.log(`Read ${content.length} bytes from ${filePath}`);
  return content;
}
...
async function main() {
  try {
    ...
    await step20();
    console.log("Step 21: Read file /Users/chietala/Code/engram/.claude/skills/improve-skill-generation/scripts/script.ts");
    await step21();
